...
class RazorpayExecutionError(Exception):
    """A payment link was not created; the action remains safe to retry."""
...
class RazorpayService:
    ...
    def create_payment_link(
        ...
        customer: Customer,
        description: str,
        force_demo_failure: bool = False
    ) -> Dict[str, Any]:
        """
        Creates a test payment link. Emulation is used only when no test credentials
        are configured; a real Razorpay API failure is never converted into a link.
        """
        client = None
        ...
        try:
            ...
            if force_demo_failure:
                raise RuntimeError("[DEMO / TESTING UTILITY] Simulated Razorpay Test Mode API failure.")
            ...
            if client:
                ...
                if not link_id or not short_url:
                    raise RuntimeError("Razorpay did not return a payment link ID and checkout URL.")
        except Exception as exc:
            # A failed execution must not mutate the action into a successful state.
            action.razorpay_link_id = None
            action.razorpay_short_url = None
            action.status = "APPROVED"
            action.executed_at = None
            audit_log = AuditLog(
                timestamp=datetime.utcnow(), goal_id=action.goal_id, opportunity_id=action.opportunity_id,
                action_id=action.id, event_type="RAZORPAY_LINK_CREATION_FAILED",
                agent_recommendation=f"Create payment link for ₹{action.final_price:,.2f}.",
                reason="Razorpay Test Mode did not create a payment link; no money action was executed.",
                proposed_amount=action.final_price, proposed_discount=action.proposed_discount_pct,
                applicable_policy="Approved action; Razorpay Test Mode execution required.",
                policy_result="EXECUTION_FAILED", human_approval="APPROVED" if action.approved_by_merchant else "AUTO_APPROVED",
                razorpay_action="[DEMO / TESTING UTILITY] Simulated execution failure" if force_demo_failure else "Razorpay Test Mode API failure",
                final_outcome="No payment link was created. The approved action can be retried safely.",
                metadata_json=json.dumps({"error": str(exc), "demo": force_demo_failure})
            )
            db.add(audit_log)
            db.commit()
            raise RazorpayExecutionError("Razorpay Test Mode could not create the payment link. No link was created; retry the approved action.") from exc

        if not client:
            # No credentials is an explicitly labelled local demo mode, not an API failure.
        ...
        """
        The link fields and EXECUTED status are set only after a real Test Mode
        response or the explicit no-credentials local demo path above.
        """
